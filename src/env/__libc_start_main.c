...
void __init_libc(void *args) {
  ...
  const char *pn = (const char *)argv_int[0];
  ...
  auxv_internal[AT_UID] = auxv_internal[AT_EUID] =
      (size_t)GetCurrentThreadUID();

  auxv_internal[AT_GID] = auxv_internal[AT_EGID] =
      (size_t)GetCurrentProcessUID();
